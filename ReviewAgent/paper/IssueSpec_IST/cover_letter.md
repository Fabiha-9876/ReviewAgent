...
classifier never saw during training. Along the way we find that LLM annotation noise on
software-engineering text is concentrated on minority classes rather than spread uniformly,
which is why correcting it against a small human anchor works at all.
...
We re-ran the comparison with the confound removed. Both arms now come from the same model,
with the same retrieved context, differing only in whether the specification appears in the
prompt, and the two arms are indistinguishable on surface features. Three raters scored all 200
blinded rows. The gain is 0.03 Likert points, p = 0.38, and the raters disagree in direction:
...
**Reproducibility.** Code, data, and models are released. An eighteen-segment verifier
recomputes every numerical claim in the results section from the saved data files, including
each negative result above, so a reviewer can check the paper against the artifact without
re-running the pipeline. We also measured and report run-to-run variance of the LLM-dependent
